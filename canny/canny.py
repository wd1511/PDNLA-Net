...
def canny(
    ...
    if torch.max(negative_idx) > 7 or torch.min(negative_idx) < 0:
        print(torch.max(negative_idx),torch.min(negative_idx),negative_idx.size())
    if torch.max(positive_idx) > 7 or torch.min(positive_idx) < 0:
        print(torch.max(positive_idx),torch.min(positive_idx),positive_idx.size())
...
class Canny(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        try:
            x = canny(input, self.low_threshold, self.high_threshold, self.kernel_size, self.sigma, self.hysteresis, self.eps)
